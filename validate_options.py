...
def ask_number_postive(): 
    validate = True
    while validate:
        try:
            id = int(input("Add the ID of the students: "))
            if id > 0:
                validate = False
            else:
...
def validate_input_product_name(): #function to validate the product name input successfully
    user_input = input("Enter the name of the students: ").strip()
    if not user_input: #if the user enter empty string, print error 
        print("ERROR: the name cannot be empty. Please enter a valid product name.")
        return validate_input_product_name()
    elif not user_input.replace(" ", "").isalpha(): #only alphabetic characters
        print("ERROR: Product name can only contain letters. Please enter a valid name.")
        return validate_input_product_name()
    else:
        return user_input


def validate_id(id, inventory):
    for students in inventory: #for to check if the product name already exists in the inventory, if exists print error
        if students['Id'] == id:
            print(f"ERROR: Product '{id}' already exists in the inventory. Please enter a different product name.")
            return False
    return True
...
def validate_input_price(): #function to validate the price input 
    try:
        price = float(input("Enter the Id of the students: "))
...
def validate_input_quantity(): #function to validate the quantity input
    ...
    while validate:
        try:
            quantity = int(input("Enter the age of students: ")) #not negative number and not zero
            if quantity <= 0:
...
def validate_input_course(): #function to validate the quantity input
    validate = True
    while validate:
        try:
            quantity = int(input("Enter the course of students: ")) #not negative number and not zero
            if quantity <= 0:
                print("ERROR: Quantity must be positive and non-zero. Please enter a valid quantity.")
            else:
                validate = False
